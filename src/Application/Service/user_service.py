...
class UserService:
    ...
    def update_user(self, user_id: int, data: dict) -> UserDomain | None:
        ...
        # Atualiza os campos permitidos (nome, email, celular)
        # O frontend não envia senha, então não precisamos lidar com isso aqui.
        if 'nome' in data:
            user_model.nome = data['nome']
        if 'email' in data:
            # Você pode querer adicionar uma verificação de e-mail duplicado aqui
        ...
        # Retorna o objeto UserDomain, como o controller espera
        return UserDomain(
            id=user_model.id,
            nome=user_model.nome,
            cnpj=user_model.cnpj,
